# This version multiplies -1 to the image stacks. As frealign is black in white.
import cupy as cp
import numpy as np
import mrcfile
import os,sys,math
...
def read_Series_Particle_NUM(pstar_line,Real_Start_Line,Real_End_Line,p_IMG_index,p_ROT_index,p_TILT_index,p_PSI_index,p_DFU_index,p_DFV_index, \
	...
	for l in range(Real_Start_Line,Real_End_Line):
		...
		#	ctf_image=ctf1.getFftwImage(False,False,False,False)
			...
			gpu_image2 = cp.asarray(-1.0*p_image1_slice_i,dtype=cp.float32)
			##########
			##	Multiply by -1 here.
			##########
			...
			for psi in range(int(360//PSI_STEP)):
