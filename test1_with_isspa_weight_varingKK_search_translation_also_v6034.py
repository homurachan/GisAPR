...
def translation_twoD_image(image,trans_X,trans_Y):
	...
	trans_x=int(cp.floor(trans_X+0.5))
	trans_y=int(cp.floor(trans_Y+0.5))
...
def rotate_fourier_image(image, angle):
	"""
	Rotate a 2D complex image in Fourier space by a given angle.

	Parameters:
	image (cp.ndarray): 2D complex array in Fourier space.
	angle (float): Rotation angle in degrees.

	Returns:
	cp.ndarray: Rotated 2D complex array in Fourier space.
	"""
...
#	offset=center
	# Apply the rotation using affine transformation
	rotated_image = affine_transform(image, cp.array(rotation_matrix), offset=offset, order=1)

	return rotated_image
def generate_twoD_distance_image(YSIZE):
	outmap=np.ones((YSIZE, YSIZE))
	for j in range(0,YSIZE):
		for i in range(0,YSIZE):
			distance=np.sqrt((j - YSIZE/2.)**2 + (i - YSIZE/2.)**2)
			if(distance < 1.0):
				distance = 1.0
		#	print (distance)
			outmap[i,j]=1.0/distance
	return outmap
...
#	Name=str(image_serial_int)+"_at_"+real_filename
	with mrcfile.new(filename,overwrite=True) as debug_mrc:
		debug_mrc.set_data((FT_array.get()).astype(np.float32))
	debug_mrc.close()
...
def read_Series_Particle_NUM(pstar_line,Real_Start_Line,Real_End_Line,p_IMG_index,p_ROT_index,p_TILT_index,p_PSI_index,p_DFU_index,p_DFV_index, \
	...
	FULL_SIZE = int((Real_End_Line-Real_Start_Line)*NUM_PSI_ROTATION)+1
	particle_array=cp.zeros((FULL_SIZE,NEW_BOXSIZE,rfft_NEW_BOXSIZE),dtype=cp.complex64)
#	particle_array=cp.zeros(rfft_NEW_BOXSIZE*NEW_BOXSIZE,dtype=cp.complex64)
#	particle_array=cp.reshape(particle_array,(1,NEW_BOXSIZE,rfft_NEW_BOXSIZE))
...
#	for l in range(p_mline,p_mline+5):
	#	print("DEBUG, RP, l=",l)
		xx=len(pstar_line[l].split())
		if (pstar_line[l].split()):
			p_PSI=0.
		#	print("RP step1 all start:", time.time()-timexxx)
			p_image_name1=str(pstar_line[l].split()[p_IMG_index])
			p_image_serial=str(p_image_name1.split('@')[0])
			p_image_serial_int=int(p_image_serial)-1
			p_filename1=str(p_image_name1.split('@')[1])
			p_real_filename = get_original_filenames(file_path=p_filename1)
			p_ROT = float(pstar_line[l].split()[p_ROT_index])
			p_TILT = float(pstar_line[l].split()[p_TILT_index])
			if(p_PSI_index!=-1):
				p_PSI = float(pstar_line[l].split()[p_PSI_index])
			if(p_DFU_index!=-1):
				# have CTF info
				p_DFU = float(pstar_line[l].split()[p_DFU_index])
				p_DFV = float(pstar_line[l].split()[p_DFV_index])
				p_DFA = float(pstar_line[l].split()[p_DFA_index])
			
		#	start reading the mrcs file.
		#	print("RP step1.1:", time.time()-timexxx)
			#######################
			#######################
			# DO NOT USE mrcfile.open. USING mrcfile.mmap could be 10x faster!!!!!
			# In fact,  mrcfile.open(p_real_filename, mode='r') as mrc2 took 2seconds, while mrcfile.mmap(p_real_filename, mode='r') as mrc2 only took 0.001seconds!!!!!!
			# by changing all mrcfile.open to mrcfile.mmap, the total time to execute 1950 particles was reduced from 4500seconds down to 400seconds!!!!!!!!! (NO translational search)
			with mrcfile.mmap(p_real_filename, mode='r') as mrc2:
				image2=mrc2.data
			...
			if(len(image2.shape)<3):
				...
				p_ZSIZE=image2.shape[0]
			if(p_XSIZE!=XSIZE or p_YSIZE!=YSIZE):
				print("X/YSIZE of models and particles mismatch!")
				break
		#	print("RP step1.2:", time.time()-timexxx)
		#	read image_serial_int@real_filename
			if(len(image2.shape)<3):
				p_image1_slice_i=image2
			else:
				p_image1_slice_i=image2[p_image_serial_int,:,:]
		#	print("RP step2:", time.time()-timexxx)
			trans_X=0
			trans_Y=0
		#	trans_X=random.randint(-TRANS_MAX,TRANS_MAX)
		#	trans_Y=random.randint(-TRANS_MAX,TRANS_MAX)
			
			#####
			# Note: relion translation = -1 * what should be. So in relion_project, --xoff -20 --yoff -30 will be equal to trans_X,trans_Y=20,30
			#### CTF correction, flipphase only.
			###################################################
			# The CTF_cupy module is completely recoded (not use cupy, still use numpy). Now the program is 4times faster. 
			if(p_DFU_index==-1):
				#for test, no CTF info
				ctf1=CTF(defocus_U=10000.0,defocus_V=5000.0,defocus_A=30.0,CS=2.7,voltage=300.0,pixel_size=NEW_APIX,XSIZE=NEW_BOXSIZE,YSIZE=NEW_BOXSIZE)
			else:
				ctf1=CTF(defocus_U=p_DFU,defocus_V=p_DFV,defocus_A=p_DFA,CS=ccss,voltage=VOLTAGE,pixel_size=NEW_APIX,XSIZE=NEW_BOXSIZE,YSIZE=NEW_BOXSIZE)
		...
		#	print("RP step3:", time.time()-timexxx)
			# ctf_image with GisSPA weight.
			#Multiply CTF. So with CTF Amplitute weighting.
			if(p_DFU_index==-1):
				ctf_image1=np.ones_like(ctf_image)
				ctf_image=ctf_image1
			gpu_ctf_image=cp.asarray(ctf_image,dtype=cp.complex64)

			norm_gpu_ctf_image=cp.linalg.norm(gpu_ctf_image)
			# translate the particle image.
			gpu_image2 = cp.asarray(p_image1_slice_i,dtype=cp.float32)
		#	gpu_image2=remove_spots_with_neighbors(gpu_image2)
			
		#	if(p_PSI_index==-1):
		#		p_PSI=random.randint(0,359)
		#	print("RP step3.0:", time.time()-timexxx)
			
			###### Currently no mask is applied
			gpu_image2_ori_bak=cp.copy(gpu_image2)
			TMPPPP=9999.
			REM_NEAREST_PSI=-9999.0
			for psi in range(360//PSI_STEP):
				DIS_PSI=np.fabs(PSI_STEP*psi-(p_PSI+360)%360)
				if(DIS_PSI<TMPPPP):
					TMPPPP=DIS_PSI
					REM_NEAREST_PSI=PSI_STEP*psi
		#	print("RP step3.01:", time.time()-timexxx)
			for psi in range(int(NUM_PSI_ROTATION)):
				gpu_image2=cp.copy(gpu_image2_ori_bak)
				particle_ROT.append(p_ROT)
				particle_TILT.append(p_TILT)
				particle_PSI.append((p_PSI+360)%360)
				...
				#	print("debug, particle_psi=", (p_PSI+360)%360," , local search psi_rot_degree=",psi_rot_degree)
				particle_fake_PSI.append(psi_rot_degree)
				
				# You rot particles, so No -1 should be multiplied here!!
				if(np.fabs(psi_rot_degree)<0.5):
					psi_rot_degree=0.5

				gpu_image2=rotate_fourier_image(gpu_image2, psi_rot_degree)
			#	print("RP step3.1 Time before particle translate: ",time.time()-timexxx)
			#	if(TRANS_MAX>0.1):
			#		gpu_image2 = translation_twoD_image(gpu_image2,trans_X,trans_Y)
					
			#	print("RP step 3.2 Time after particle translate: ",time.time()-timexxx)
				# Shift in real space, interger only.
				if(not do_discard_mask):
					gpu_image2 = gpu_image2*gpu_mask
			#	print("gpu_image2.dtype",gpu_image2.dtype,"gpu_ctf_image.dtype",gpu_ctf_image.dtype,"norm_gpu_ctf_image.dtype",norm_gpu_ctf_image.dtype)
				# start psi rot the particles
			#	gpu_image2=rotate_fourier_image(gpu_image2, p_PSI)
				############
				#	rotate_fourier_image rotate image clockwise. So no -1 should be multiplied here.
				############
			#	Namex=str(p_image_serial_int)+"_at_"+p_real_filename
				fft2=cp.fft.fft2(gpu_image2)
				fft2_shift=cp.fft.fftshift(fft2)
				if(NEW_BOXSIZE!=ORIGINAL_BOXSIZE):
					fft2_shift=fft2_shift[ORIGINAL_BOXSIZE//2-NEW_BOXSIZE//2: ORIGINAL_BOXSIZE//2+NEW_BOXSIZE//2,ORIGINAL_BOXSIZE//2-NEW_BOXSIZE//2: ORIGINAL_BOXSIZE//2+NEW_BOXSIZE//2,]
				Corr_fft2_shift=fft2_shift*gpu_ctf_image
				#### fft2 to rfft2 START
				Corr_fft2_shift=cp.fft.fftshift(Corr_fft2_shift)
				Corr_fft2_shift=Corr_fft2_shift[:, :Corr_fft2_shift.shape[1]//2 + 1]
				#### fft2 to rfft2 END

				Corr_fft2_shift_conj=cp.conj(Corr_fft2_shift)
				norm_Corr_fft2_shift=cp.linalg.norm(Corr_fft2_shift_conj)
				Corr_fft2_shift_conj=Corr_fft2_shift_conj/norm_Corr_fft2_shift
				particle_array [FULL_SIZE,:,:]= Corr_fft2_shift_conj
			#	expanded_array = Corr_fft2_shift_conj[cp.newaxis, :, :]
			#	particle_array = cp.concatenate((particle_array, expanded_array), axis=0)
				FULL_SIZE += 1
			#	print("RP step 4 finish read one: ",time.time()-timexxx)
			#	print(f"Current device memory usage in line 291: {get_current_memory_usage():.2f} MB")
			#	particle_norm_array = cp.append(particle_norm_array, norm_Corr_fft2_shift)
			cp.cuda.Stream.null.synchronize()	
			del fft2
			del fft2_shift
			del Corr_fft2_shift
			del gpu_ctf_image
			del Corr_fft2_shift_conj
		#	del expanded_array
			del gpu_image2_ori_bak
			if(not do_local_search):
				cp.get_default_memory_pool().free_all_blocks()
	return particle_array,particle_norm_array,particle_ROT,particle_TILT,particle_PSI,particle_T_X,particle_T_Y,particle_LINE,particle_fake_PSI,NUM_PSI_ROTATION
...
def process_chunk(chunkA, chunkB):
	...
	CA=cp.copy(chunkA)
	CB=cp.copy(chunkB)
	result = CA[:, cp.newaxis, :, :] * CB[cp.newaxis, :, :, :]
...
#	print("Search start:", time.time()-timexx)
	for N in range(NUMBER_OF_PARTICLES):
		# grab the slices from three_d_array.
	#	print(NUMBER_OF_PARTICLES)
	#	print(set_particle_LINE[N])
	#	print(chopped_local_search_SN[N][0])
	...
	#	print("Search step1:", time.time()-timexx)
		# Because Model_Psi is all zero, no need to use it.
		# Note: The Chopped_3D_Array no longer contains the useless first layer.
	...
	#	print("Search step2:", time.time()-timexx)
	...
	#	print("Search step3:", time.time()-timexx)
		cp.cuda.Stream.null.synchronize()
	#	print("Search step3.00sync:", time.time()-timexx)
	#	cp.get_default_memory_pool().free_all_blocks()
	###########
	#	NO need to free all the memory here.
	#	print("Search step3.00free:", time.time()-timexx)
		time1=time.time()
		SCORE_MAX=-9999.0
		REM_PSI=-9999999.
		REM_I=-1
	...
	#	cut_FR=final_result[:, J_lower_limit:J_upper_limit,:]
		cut_FR=final_result
	#	print("Search step3.001:", time.time()-timexx)
		# No need to cut.
	#	print("Search step3.0:", time.time()-timexx)
		reshaped_result = cut_FR.reshape(-1, final_result.shape[2])
	#	print("Search step3.01:", time.time()-timexx)
		# Compute the maximum values along the last two axes
		max_values = cp.array(reshaped_result[:,0])
	#	print("Search step3.02:", time.time()-timexx)
		peak_value = cp.max(max_values)
	#	print("Search step3.03:", time.time()-timexx)
		indices_where = cp.where(cut_FR[:,:,0] == peak_value)
	#	print("Search step3.1:", time.time()-timexx)
	...
	#	print("Search step3.2:", time.time()-timexx)
		...
		if(Angle_distance_CC2RELION<ACCURACY):
			CC_DIS_ACCU=1
	#	print("Search step3.3:", time.time()-timexx)
		...
		if(output_Z_L5):
			...
			del cp_cut_FR
		del final_result
	#	print("Search step4 END:", time.time()-timexx)
#	print("step4.2 END main iteration:", time.time()-time0)
	cp.cuda.Stream.null.synchronize()
	time2=time.time()
#	print(f"Current device memory usage in line 407: {get_current_memory_usage():.2f} MB")
...
#	three_d_array=cp.zeros(rfft_NEW_BOXSIZE*NEW_BOXSIZE,dtype=cp.complex64)
#	three_d_array=cp.reshape(three_d_array,(1,NEW_BOXSIZE,rfft_NEW_BOXSIZE))
#	peak_devmem_usage = max(peak_devmem_usage, get_current_memory_usage())
#	print(f"Peak device memory usage in line 563: {peak_devmem_usage:.2f} MB")

	Model_Rot=[]
	Model_Tilt=[]
#	Model_Rot.append(9999.0)
#	Model_Tilt.append(9999.0)
	Model_Psi=[]
#	Model_Psi.append(9999.0)
	...
	for l in range(mline,len(instar_line)):
		...
		#	expanded_array = conjugate_fft1[cp.newaxis, :, :]
			three_d_array[FULL_SIZE,:,:]=conjugate_fft1
		#	three_d_norm_array=cp.append(three_d_norm_array,NORM_FFT)
		#	three_d_array = cp.concatenate((three_d_array, expanded_array), axis=0)
		#	peak_devmem_usage = max(peak_devmem_usage, get_current_memory_usage())
		#	print(f"Peak device memory usage in line 635: {peak_devmem_usage:.2f} MB")
	...
	#	print(Full_Particle_Star_Info_To_Search_3D_Stack_SN[0])
		if(IS_A_CONTINUE_RUN):
			TEMP_Full_Particle_Star_Info_To_Search_3D_Stack_SN=Full_Particle_Star_Info_To_Search_3D_Stack_SN[START_LINE-p_mline:]
			...
			# remove the first unused lines.
		print("End reading full star and find the near orientations. Time = ",time.time()-TIMEX)
	#######################
	##
	## Full_Particle_Star_Info_To_Search_3D_Stack_SN[0][0] is the particle_SN, the rests are the actual slice number of 3D_array.
	##
	#######################
	# So if not doing local search, Full_Particle_Star_Info_To_Search_3D_Stack_SN = null list.
	time1=time.time()
#	print("three_d_array.dtype=",three_d_array.dtype)
#	peak_devmem_usage = max(peak_devmem_usage, get_current_memory_usage())
#	print(f"Peak device memory usage in line 657: {peak_devmem_usage:.2f} MB")
	...
	HOW_MANY_ITERATION=int(np.ceil((len(pstar_line)-START_LINE)/Series_Particle_NUM))
	INTER_COUNT=0
	FREE_BLOCK_ROUND_IN_LOCAL_SEARCH = 16
#	print(HOW_MANY_ITERATION)
	
	for N in range(0,HOW_MANY_ITERATION):
		timex=time.time()
	#	print("Step0 = ",round(time.time()-timex,4)," seconds")
		...
		particle_array,particle_norm_array,particle_ROT,particle_TILT,particle_PSI,particle_T_X,particle_T_Y,particle_LINE,particle_fake_PSI,NUM_PSI_ROTATION = \
		...
		p_DFA_index,TRANS_MAX,ORIGINAL_BOXSIZE,NEW_BOXSIZE,XSIZE,YSIZE,ccss,VOLTAGE,do_discard_mask,gpu_mask,NEW_APIX,kk,time0,p_mline,PSI_STEP,do_local_search,local_search_angular_distance)
		cp.cuda.Stream.null.synchronize()
	#	print(particle_array.shape,particle_array.dtype)
	#	print("Step1 readparticle= ",round(time.time()-timex,4)," seconds")
		if(do_local_search):
			if(INTER_COUNT%FREE_BLOCK_ROUND_IN_LOCAL_SEARCH==0):
				cp.get_default_memory_pool().free_all_blocks()
		else:
			cp.get_default_memory_pool().free_all_blocks()
	#	Chop the Full_Particle_Star_Info_To_Search_3D_Stack_SN
	...
	#	print("Step2 chopLSSN= ",round(time.time()-timex,4)," seconds")
	########################################################################
	########################################################################
	##
	## Now I only need to modify search_stack_of_particles. It needs to read do_local_search and read Full_Particle_Star_Info_To_Search_3D_Stack_SN.
	## Oh, the psi part of particles should also be changed, isnt it? (done)
	########################################################################
	########################################################################
		...
		cp.cuda.Stream.null.synchronize()
	#	print("Step3 search= ",round(time.time()-timex,4)," seconds")
		for FF in range(0,len(RESULTS)):
			out.write(str(RESULTS[FF])+"\n")
		out.close()
		del particle_array
		if(do_local_search):
			if(INTER_COUNT%FREE_BLOCK_ROUND_IN_LOCAL_SEARCH==0):
			...
			cp.get_default_memory_pool().free_all_blocks()
	#	print(f"Current device memory usage in line 710: {get_current_memory_usage():.2f} MB")
		out=open(output,"a")
	#	if((N)%SF ==0 and (N)>0):
	#		print("Execution "+str(Series_Particle_NUM*SF)+" particles, time = ",round(time.time()-time0,4)," seconds")
...
def chop_local_search_SN_Function_old(Full_Particle_Star_Info_To_Search_3D_Stack_SN, particle_LINE):
	# Step 1: Get unique serial numbers from particle_LINE
	unique_serial_numbers = sorted(list(set(particle_LINE)))
	# Warning!!!! set(particle_LINE) is in RANDOM ORDER!!!!!!!!!!!!
	# Step 2: Initialize the list to store the extracted data
	chopped_local_search_SN = []

	# Step 3: Filter and remove elements with matching serial numbers
	# Using list comprehension to create new filtered list
	remaining_data = []
	for entry in Full_Particle_Star_Info_To_Search_3D_Stack_SN:
		serial_number = entry[0]
		if serial_number in unique_serial_numbers:
			chopped_local_search_SN.append(entry)
		else:
			remaining_data.append(entry)
	# Update Full_Particle_Star_Info_To_Search_3D_Stack_SN to exclude the saved entries
	Full_Particle_Star_Info_To_Search_3D_Stack_SN[:] = remaining_data
...
#	print("len(particle_LINE)=",len(particle_LINE))
#	print("particle_LINE[0]=",particle_LINE[0])
#	print("len(unique_serial_numbers)=",len(unique_serial_numbers))
#	print("unique_serial_numbers[0]=",unique_serial_numbers[0])
#	print("unique_serial_numbers[-1]=",unique_serial_numbers[-1])
	
#	print("len(Full_Particle_Star_Info_To_Search_3D_Stack_SN)=",len(Full_Particle_Star_Info_To_Search_3D_Stack_SN))
	# Step 2: Initialize the list to store the extracted data
	chopped_local_search_SN = []
...
#		print("serial_number = ",serial_number)
		# Stop early if the current serial_number is less than the smallest in unique_serial_numbers
	#	if serial_number < unique_serial_numbers[-1]:
	#		break
