...
project3d_and_whiten_path = "python project3d_and_whiten.py"
...
def create_pdb2projection(args):
	...
	apix=args.apix
	APIX=convert_number_to_filename_2(apix)
	res=2.0*apix
	...
	pdb2mrc_command_line+=(pdb2mrc_path+" --i "+PDB_filename+" --o "+mrc_filename+" --box "+str(boxsize)+" --apix "+str(apix)+" --res "+str(res))
	if(args.yflip):
...
#	b.write(sh1_command_line)
	for j in range(NN):
		sh2_command_line+="python "+search_script+" --i "+star_filename_prefix+".star --p "+particle_file+" --FSC "+ fsc_file+" --oriboxsize "+str(boxsize)+" --newboxsize "+str(newboxsize)+" --apix "+str(apix)+" --transRange "+transRange+" --voltage "+voltage+" --cs "+cs \
		+" --maskRadius "+str(maskRadius)+" --maskEdge "+str(maskEdge)+" --psiStep "+psiStep+" --kk "+kk
		if(args.ignoreFSC):
		...
		if(COUNT_FOR_GPU>=ngpus):
			COUNT_FOR_GPU=0
		if(do_local_search):
			sh2_command_line+=" --doLocalSearch --localRange "+local_stepsize
