	...
	def run_iterations(self, n):
		...
		for iteration in range(n):
			...
			with concurrent.futures.ProcessPoolExecutor(max_workers=self.max_workers) as executor:
				...
				scores = list(executor.map(func_partial, self.positions))
...
if __name__ == "__main__":
	...
	run_continue=False
	continue_rounds=20
	continue_run_filename="my_pso_state_round_30.npz"
	# Run optimization
	if(not run_continue):
		pso.optimize()
		print("Finish PSO.")
		...
		try:
			pso_restored = pso.from_file(filename=continue_run_filename, objective_func=func)
