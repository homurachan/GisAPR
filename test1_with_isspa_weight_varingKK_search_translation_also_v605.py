...
def read_Series_Particle_NUM(pstar_line,Real_Start_Line,Real_End_Line,p_IMG_index,p_ROT_index,p_TILT_index,p_PSI_index,p_DFU_index,p_DFV_index, \
	...
	for l in range(Real_Start_Line, Real_End_Line):
		...
		gpu_image2_ori_bak = cp_asarray(p_image1_slice_i, dtype=cp.float32)
...
def find_max_and_position(Arr):
	...
	RESULT[:, :, 0] = max_values.astype(cp.float32, copy=False)
	RESULT[:, :, 1] = (max_indices // ny).astype(cp.float32, copy=False)
	RESULT[:, :, 2] = (max_indices % ny).astype(cp.float32, copy=False)
	return RESULT
